...
public:
	...
	TransferMatrixResources(
		const std::string & FGD_path, 
		const std::string& FGD_Belcour_path, 
		const std::string& FGD_Coeff_path,
		const std::wstring& FGD_4D_path, 
		const std::wstring& GD_path, 
		const std::wstring & TIR_path);
	...
	TextureRef FGD_Belcour_Coefficients; //precomputed coefficients for belcour FGD.
	Texture3D FGD_4D_LUT;


	static constexpr int32_t NUM_LUTS = 6;
